    ...
    return (
      <div className="absolute z-10 flex w-full items-center justify-between px-8 py-4">
        <Link href={'/account'} className="flex items-center gap-2">
          <span className="text-lg font-regular text-slate-100">Cuenta Sueldo</span>
          <ChevronDownIcon className="h-5 text-slate-100" />
        </Link>

        <div className="flex items-center gap-2">
          <MoonIcon className="h-5 text-slate-100"/>
          <UserIcon className="h-5 text-slate-100"/>
          <Cog6ToothIcon className="h-5 text-slate-100"/>
        </div>
      </div>
    );
  };
